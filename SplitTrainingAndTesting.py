...
def split_set(X,y,names,TestNames):
    ...
    assert (type(TestNames) is list),'TestNames should be list'
...
def split_set_random(X,y,names,test_number,seed):
    ...
    (N,p) = X.shape
    assert (test_number<N),"Select test_number to be less than total number of samples in set. test_number={0}, number of samples={1}".format(test_number,N)
    idxs = np.arange(N)
